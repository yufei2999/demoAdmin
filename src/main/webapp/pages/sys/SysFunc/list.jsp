...
<form id="queryForm" name="queryForm" action="${ctx}/sysFunc/listAjax" method="post" style="display: inline;">
	...
	<div id="ts">
		<div>
			<table style="font-size: 12px">
				<tr>
					<td><%=SysFunc.ALIAS_FUNC_NAME%></td>
					<td>
						<input class="easyui-textbox" id="funcName" name="funcName"/>
                    </td>
					<td><%=SysFunc.ALIAS_FUNC_CODE%></td>
					<td>
						<input class="easyui-textbox" id="funcCode" name="funcCode"/>
                    </td>
					<td><%=SysFunc.ALIAS_URL%></td>
					<td>
						<input class="easyui-textbox" id="url" name="url"/>
                    </td>
				</tr>
				<tr>
					<td><%=SysFunc.ALIAS_PARENT_ID%></td>
					<td>
						<input class="easyui-textbox" id="parentId" name="parentId"/>
                    </td>
					<td><%=SysFunc.ALIAS_PARENT_IDS%></td>
					<td>
						<input class="easyui-textbox" id="parentIds" name="parentIds"/>
                    </td>
					<td><%=SysFunc.ALIAS_REMARK%></td>
					<td>
						<input class="easyui-textbox" id="remark" name="remark"/>
                    </td>
				</tr>
				<tr>
					<td><%=SysFunc.ALIAS_CREATOR%></td>
					<td>
						<input class="easyui-textbox" id="creator" name="creator"/>
                    </td>
					<td><%=SysFunc.ALIAS_CREATE_TIME%></td>
					<td>
					...
					<td><%=SysFunc.ALIAS_LAST_MODIFIER%></td>
					<td>
						<input class="easyui-textbox" id="lastModifier" name="lastModifier"/>
                    </td>
				</tr>
				<tr>
					<td><%=SysFunc.ALIAS_LAST_MODIFY_TIME%></td>
					<td>
...
<script type="text/javascript">
	...
	function show() {
		...
		if (selected){
			location.href="${ctx}/sysFunc/show?id="+selected.id;
		}
	}
	function add() {
		location.href="${ctx}/sysFunc/create";
	}
	function edit() {
		var selected = $('#userDataGrid').datagrid('getSelected');
		if (selected){
			location.href="${ctx}/sysFunc/edit?id="
			+selected.id;
		}
	}
	// 删除当前行
	...
	var option = {
			...
			collapsible:true,
			url: "${ctx}/sysFunc/listAjax",//从远程站点请求数据的 URL
			loadMsg:"正在努力加载数据...",
			remoteSort: false,
			...
			columns:[[
				{field:'ck',checkbox:true}
					,{field:'id',title:'<%=SysFunc.ALIAS_ID%>',width:120,sortable:true}
					,{field:'funcName',title:'<%=SysFunc.ALIAS_FUNC_NAME%>',width:120,sortable:true}
					,{field:'funcCode',title:'<%=SysFunc.ALIAS_FUNC_CODE%>',width:120,sortable:true}
					,{field:'url',title:'<%=SysFunc.ALIAS_URL%>',width:120,sortable:true}
					,{field:'parentId',title:'<%=SysFunc.ALIAS_PARENT_ID%>',width:120,sortable:true}
					,{field:'parentIds',title:'<%=SysFunc.ALIAS_PARENT_IDS%>',width:120,sortable:true}
					,{field:'remark',title:'<%=SysFunc.ALIAS_REMARK%>',width:120,sortable:true}
					,{field:'creator',title:'<%=SysFunc.ALIAS_CREATOR%>',width:120,sortable:true}
					,{field:'createTimeString',title:'<%=SysFunc.ALIAS_CREATE_TIME%>',width:120,sortable:true}
					,{field:'lastModifier',title:'<%=SysFunc.ALIAS_LAST_MODIFIER%>',width:120,sortable:true}
					,{field:'lastModifyTimeString',title:'<%=SysFunc.ALIAS_LAST_MODIFY_TIME%>',width:120,sortable:true}
					,{field:'deleted',title:'<%=SysFunc.ALIAS_DELETED%>',width:120,sortable:true}
					,{field:'action',title:'操作',sortable:false,formatter:function(value,row,index){
						return '<a href="${ctx}/sysFunc/show?id=' + row.id + '">查看</a>&nbsp;' +
							'<a href="${ctx}/sysFunc/edit?id=' + row.id + '">修改</a>&nbsp;' +
