...
<html>
<head>
	<title><%=SysRoleFunc.TABLE_ALIAS%> 维护</title>
...
<form id="queryForm" name="queryForm" action="${ctx}/sysRoleFunc/listAjax" method="post" style="display: inline;">
	...
	<div id="ts">
		<div>
			<table style="font-size: 12px">
				<tr>
					<td><%=SysRoleFunc.ALIAS_ROLE_ID%></td>
					<td>
						<input class="easyui-textbox" id="roleId" name="roleId"/>
                    </td>
					<td><%=SysRoleFunc.ALIAS_FUNC_ID%></td>
					<td>
						<input class="easyui-textbox" id="funcId" name="funcId"/>
                    </td>
					<td><%=SysRoleFunc.ALIAS_CREATOR%></td>
					<td>
						<input class="easyui-textbox" id="creator" name="creator"/>
                    </td>
				</tr>
				<tr>
					<td><%=SysRoleFunc.ALIAS_CREATE_TIME%></td>
					<td>
					...
					<td><%=SysRoleFunc.ALIAS_LAST_MODIFIER%></td>
					<td>
						<input class="easyui-textbox" id="lastModifier" name="lastModifier"/>
                    </td>
					<td><%=SysRoleFunc.ALIAS_LAST_MODIFY_TIME%></td>
					<td>
				...
				<tr>
					<td><%=SysRoleFunc.ALIAS_DELETED%></td>
					<td>
...
<script type="text/javascript">
	...
	function show() {
		...
		if (selected){
			location.href="${ctx}/sysRoleFunc/show?id="+selected.id;
		}
	}
	function add() {
		location.href="${ctx}/sysRoleFunc/create";
	}
	function edit() {
		var selected = $('#userDataGrid').datagrid('getSelected');
		if (selected){
			location.href="${ctx}/sysRoleFunc/edit?id="
			+selected.id;
		}
	}
	// 删除当前行
	...
	var option = {
			...
			collapsible:true,
			url: "${ctx}/sysRoleFunc/listAjax",//从远程站点请求数据的 URL
			loadMsg:"正在努力加载数据...",
			remoteSort: false,
			...
			columns:[[
				{field:'ck',checkbox:true}
					,{field:'id',title:'<%=SysRoleFunc.ALIAS_ID%>',width:120,sortable:true}
					,{field:'roleId',title:'<%=SysRoleFunc.ALIAS_ROLE_ID%>',width:120,sortable:true}
					,{field:'funcId',title:'<%=SysRoleFunc.ALIAS_FUNC_ID%>',width:120,sortable:true}
					,{field:'creator',title:'<%=SysRoleFunc.ALIAS_CREATOR%>',width:120,sortable:true}
					,{field:'createTimeString',title:'<%=SysRoleFunc.ALIAS_CREATE_TIME%>',width:120,sortable:true}
					,{field:'lastModifier',title:'<%=SysRoleFunc.ALIAS_LAST_MODIFIER%>',width:120,sortable:true}
					,{field:'lastModifyTimeString',title:'<%=SysRoleFunc.ALIAS_LAST_MODIFY_TIME%>',width:120,sortable:true}
					,{field:'deleted',title:'<%=SysRoleFunc.ALIAS_DELETED%>',width:120,sortable:true}
					,{field:'action',title:'操作',sortable:false,formatter:function(value,row,index){
						return '<a href="${ctx}/sysRoleFunc/show?id=' + row.id + '">查看</a>&nbsp;' +
							'<a href="${ctx}/sysRoleFunc/edit?id=' + row.id + '">修改</a>&nbsp;' +
